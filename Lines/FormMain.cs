...
namespace linesk
{
    ...
    public partial class FormMain : Form
    {
        ...
        private void Form1_Paint(object sender, PaintEventArgs e)
        {
            ...
            g.DrawString("Сейчас играет - " + NamePlayer, font, new SolidBrush(Color.Purple), x, y);
            ...
            for (int i = 0; i < GameSet.DRH.Length; i++)
            {
                ...
                {
                    g.DrawString("Набранные тобой очки - " + GameSet.DRH[i].Score.ToString(), font, new SolidBrush(Color.Blue), x, y);
                    break;
                }
            }


            g.DrawString("Наши рекордсмены:", font, new SolidBrush(Color.Gold), x, y + 20);
        ...
        bool CheckList(List<GraphItem> parseList)
        {
            ...
            { 
                ...
                if (hideList.Count >= 5)
                {
                    ...
                    Stream res = this.GetType().Assembly.GetManifestResourceStream("linesk.gun1.wav");
        ...
        void ParseDataPlayers()
        {
            ...
            for (int i = 0; i < GameSet.DRH.Length; i++)
            {
                if (GameSet.DRH[i].CurrentPlayer == true)
                {
                    ...
                    if (i < index && i == 0)
                    {
                        // Добавить музыку поздрвления
                        Stream res = (Stream)this.GetType().Assembly.GetManifestResourceStream("linesk.gun1.wav");
                        SoundPlayer sp = new SoundPlayer(res);
                        sp.Play();
                    }
                    break;
